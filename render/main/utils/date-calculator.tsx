const DateCalculator = {
    ...
    diffWeek: (source: Date, target: Date): number => {
        return Math.floor(Math.abs((source.getTime() - target.getTime()) / (1000 * 60 * 60 * 24 * 7)));
    },

    diffDay: (source: Date, target: Date): number => {
        return Math.floor(Math.abs((source.getTime() - target.getTime()) / (1000 * 60 * 60 * 24)));
    },

    diffHours: (source: Date, target: Date): number => {
        return Math.floor(Math.abs((source.getTime() - target.getTime()) / (1000 * 60 * 60)));
    },

    diffMinutes: (source: Date, target: Date): number => {
        return Math.floor(Math.abs((source.getTime() - target.getTime()) / (1000 * 60)));
    },

    diffSeconds: (source: Date, target: Date): number => {
        return Math.floor(Math.abs((source.getTime() - target.getTime()) / 1000));
    },
